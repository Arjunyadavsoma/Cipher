...
class KnowledgeRepository {
  ...
  /// Get a single entry.
  Future<KnowledgeEntry?> getEntry(String layer, String entryId) async {
    ...
    return KnowledgeEntry.fromMap(doc.id, doc.data()!);
  }

  /// Create a new entry.
  Future<void> addEntry(String layer, KnowledgeEntry entry) async {
    await _entries(layer).doc(entry.id).set(entry.toMap());
  }

  /// Update an existing entry.
  Future<void> updateEntry(String layer, KnowledgeEntry entry) async {
    await _entries(layer).doc(entry.id).update(entry.toMap());
  }

  /// Delete an entry.
  Future<void> deleteEntry(String layer, String entryId) async {
    await _entries(layer).doc(entryId).delete();
  }

  /// Updates lastUsedAt.
  Future<void> touchLastUsed(String layer, String entryId) async {
    await _entries(
      layer,
    ).doc(entryId).update({'lastUsedAt': DateTime.now().toIso8601String()});
  }

  /// Search entries by fact.
  Future<List<KnowledgeEntry>> searchByFact(String layer, String query) async {
